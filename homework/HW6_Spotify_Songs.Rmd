...
spotify |>
  ggplot() +
  geom_histogram(aes(x = danceability), binwidth = 0.10)
```
...
spotify |>
   ggplot() +
  geom_histogram(aes(x = danceability), binwidth = 0.10) +
  geom_vline(aes(xintercept = mean(danceability)), color = "pink", linetype = "dashed", size = 1) 
...
spotify |>
  ...
  geom_histogram(aes(x = danceability), binwidth = 0.10) +
  geom_vline(aes(xintercept = mean(danceability)), color = "pink", linetype = "dashed", size = 1) +
  geom_vline(aes(xintercept = mean(danceability) - sd(danceability)), color = "purple", linetype = "dashed", size = 1) +
  geom_vline(aes(xintercept = mean(danceability) + sd(danceability)), color = "purple", linetype = "dashed", size = 1)
```
...
Based on the histogram the distribution is pretty symmetrical which means that the data stays relatively close to the mean, though it does have a little bit of a negative skew as seen by the data on the left side. The mean tells us that the average danceability of a song is about 0.67 and the standard deviation of 0.16 shows the variation from the average.

A story idea from this data could be to compare the danceability of songs on Spotify with those on other music streaming platforms, exploring whether danceability is a factor that influences listener preferences and contributes to subscriber growth.
