...
There are about 10 daily calls related to 911 overdoses each day with the most calls being 23 and the least being 1. There are a few outlier going above the mean of 10 that nearly doubles the mean. It is hard to see a clear pattern through this chart since there are so many entries
...
The days above are the data points that are two standard deviations or more (95%) away from the mean

If we look at the unusual days in the data set it shows how some days have an abnormal amount of calls in comparison to the normal data. Based on the z score we can see that there is one day (2022-10-04	) that is considered very unusual meaning it is only about 0.3% within the normal distribution set. The other 17 days that are noted by the z score is considered unusual. These results indicate that the call volumes on these days are significantly higher than what would be expected under normal conditions.
...
Saturday saw the highest spike in overdose calls.
...
Our analysis of overdose call data shows days with strikingly high and unusually low call volumes. For example, October 4, 2022, saw 23 calls which is far above the typical daily count, while days like June 11, 2022, and February 6, 2023, had only a single call. 

Comparing these extremes could show patterns or causes that influence overdose incidents, providing insight into when and why spikes occur, allowing journalist to dive deeper intp public health concerns and how it impacts the community.