...
1. Social Media...If you were to track a study of younger generations social media usage on an app and they use Facebook instead of one that is more common with younger generations, such as Instagram or TikTok

2. Gathering data about students who are commuters versus live on campus. If you do this data here as a state school you would find more data capturing commuters, whereas with a private school you might see completly different data supporting the other side.

When a reporter is working on a story they have to explicitly state the conditions the data was gathered in so that readers known exactly what and who they collected from. A statistical meausure they should use is margin of error where as one they should avoid is relying heavily on the mean. For example, if we focus primarily on the mean for example 2, there would be a false narritive protrayed from the data. You can also focus on median instead of mean.
...
Compared to other teams, Maryland is in the 95th percentile in terms of total aces. This means that 5% of other teams scores are higher than Maryland. Being this high in the percentile means that they are scoring more aces than 95% of other teams, but falls short of 5% of other teams 
...
The data is skewed more to the left. About 68% of the data falls within the first standard deviation.
